...
int ParseJPEGMetadata(JPEGMetadata *self,const void *bytes,size_t length)
{
	...
	for(;;)
	{
		...
		if(!ptr) return JPEGMetadataParsingFailed;
		...
		switch(*ptr++)
		{
			...
			case 0xc4: // Define huffman table
			{
				...
				while(ptr+17<=next)
				{
					...
					if(class!=0 && class!=1) return JPEGMetadataParsingFailed;
					if(index>=4) return JPEGMetadataParsingFailed;

					self->huffmandefined[class][index]=true;
					...
					if(ptr+totalcodes>next) return JPEGMetadataParsingFailed;

					DebugPrint(" > %s table at %d with %d codes\n",class==0?"DC":"AC",index,totalcodes);

					memset(&self->huffmantables[class][index],0,sizeof(JPEGHuffmanTable));
			...
			case 0xdb: // Define quantization table(s)
			{
				...
				while(ptr+1<=next)
				{
					...
					self->quantizationdefined[index]=true;

					if(precision==0)
					{
						DebugPrint(" > 8 bit table at %d\n",index);

						if(ptr+64>next) return JPEGMetadataParsingFailed;
					...
					else if(precision==1)
					{
						...
						if(ptr+128>next) return JPEGMetadataParsingFailed;
						for(int i=0;i<64;i++) self->quantizationtables[index].c[i]=ParseUInt16(&ptr[2*i]);
						ptr+=128;
					}
					else return JPEGMetadataParsingFailed;
				}
			...
			case 0xc1: // Start of frame 1
			{
				...
				if(size<8) return JPEGMetadataParsingFailed;
				self->bits=ptr[2];
				self->height=ParseUInt16(&ptr[3]);
				self->width=ParseUInt16(&ptr[5]);
				self->numcomponents=ptr[7];

				if(self->numcomponents<1 || self->numcomponents>4) return JPEGMetadataParsingFailed;
				if(size<8+self->numcomponents*3) return JPEGMetadataParsingFailed;
				...
				for(int i=0;i<self->numcomponents;i++)
				{
					...
					self->components[i].quantizationindex=ptr[10+i*3];
					self->components[i].quantizationtable=&self->quantizationtables[self->components[i].quantizationindex];
					...
					self->components[i].quantizationindex);
				}
			...
			case 0xda: // Start of scan
			{
				...
				if(self->numscancomponents<1 || self->numscancomponents>4) return JPEGMetadataParsingFailed;
				if(size<6+self->numscancomponents*2) return JPEGMetadataParsingFailed;
				...
				for(int i=0;i<self->numscancomponents;i++)
				{
					...
					if(!component) return JPEGMetadataParsingFailed;

					self->scancomponents[i].component=component;

					self->scancomponents[i].dcindex=ptr[4+i*2]>>4;
					self->scancomponents[i].acindex=ptr[4+i*2]&0x0f;
					self->scancomponents[i].dctable=&self->huffmantables[0][self->scancomponents[i].dcindex];
					self->scancomponents[i].actable=&self->huffmantables[1][self->scancomponents[i].acindex];
					...
					self->scancomponents[i].dcindex,
					self->scancomponents[i].acindex);
				}

				if(ptr[3+self->numscancomponents*2]!=0) return JPEGMetadataParsingFailed;
