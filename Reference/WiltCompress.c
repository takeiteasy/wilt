...
int typeshift,int litshift,int lengthshift1,int lengthshift2,int offsshift1,int offsshift2)
{
	...
	while(pos<size)
	{
		...
		if(FindDictionaryMatch(&dict,pos,&length,&offs))
		{
			...
			WriteUniversalCode(&comp,(pos-offs)-1,offsweights1,offsshift1,offsweights2,offsshift2);
		...
		else
		{
			...
			for(int i=7;i>=0;i--)
			{
				WriteBitAndUpdateWeight(&comp,(val>>i)&1,&literalbitweights[(val|0x100)>>(i+1)],litshift);
			}
